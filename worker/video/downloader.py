...
class VideoDownloader:
    ...
    def _validate_format_availability(
        self, url: str, format_id: Optional[str]
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(validate_opts) as ydl:
                ...
                logger.info(
                    f"Available formats: {available_formats[:10]}..."
                )  # Show first 10
                ...
                if format_id in available_formats:
                    ...
                    logger.warning(
                        f"⚠️ Format {format_id} NOT available! Backend provided incorrect format."
                    )
                    return "best[height<=720]/best"
    ...
    async def download(
        self, url: str, format_id: Optional[str] = None, temp_dir: Optional[Path] = None
    ) -> Path:
        """
        ...
        for i, config in enumerate(self.config_attempts):
            try:
                ...
                self.progress_tracker.update(
                    progress_base, stage=f"Download attempt {i+1}/5..."
                )
                ...
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    self.progress_tracker.update(
                        progress_base + 1,
                        stage=f"Extracting video info (attempt {i+1})...",
                    )
                    ...
                    requested_format = (
                        format_id if format_id and format_id != "None" else "auto"
                    )

                    logger.info(
                        f"🎬 Downloaded format: {actual_format_used}, resolution: {actual_resolution}"
                    )
                    logger.info(
                        f"🎬 Requested: {requested_format}, Got: {actual_format_used}"
                    )

                    if (
                        format_id
                        and format_id != "None"
                        and actual_format_used != format_id
                    ):
                        logger.warning(
                            f"⚠️ Format mismatch! Requested {format_id}, got {actual_format_used}"
                        )
                ...
                if "Requested format is not available" in str(e):
                    logger.warning(
                        f"🎬 Format {format_id} confirmed unavailable - backend metadata was wrong"
                    )
        ...
        if not download_success:
            ...
            raise DownloadError(
                ...
                job_id=self.progress_tracker.job_id,
                details={
                    "attempts": len(self.config_attempts),
        ...
        if not downloaded_files:
            raise DownloadError(
                "No file downloaded", job_id=self.progress_tracker.job_id
            )
    ...
    def _validate_downloaded_file(self, file_path: Path) -> None:
        ...
        if file_size == 0:
            raise DownloadError(
                "Downloaded file is empty", job_id=self.progress_tracker.job_id
            )
        ...
        if file_path.suffix.lower() not in valid_extensions:
            logger.warning(
                f"⚠️ Unexpected file extension: {file_path.suffix}. Proceeding with FFmpeg validation."
            )
