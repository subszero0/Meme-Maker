    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for i, fmt in enumerate(formats):
                logger.info(f"Format {i}: {fmt.get('format_id')} - {fmt.get('ext')} - {fmt.get('resolution', 'audio')} - {fmt.get('filesize_approx', 'Unknown size')}")
            ...
            try:
                ...
                if downloaded_files:
                    for file in downloaded_files:
                        ...
                        logger.info(f"✅ Downloaded: {file.name} ({file_size} bytes, {file.suffix})")
                        
                        # Check if it's actually a video file
                        if file.suffix.lower() in ['.html', '.mhtml', '.htm']:
                            logger.error(f"❌ Downloaded HTML instead of video: {file.name}")
