...
try:
    ...
    logger.error(
        f"❌ Could not import from backend 'app'. Ensure PYTHONPATH is set correctly."
    )
        ...
        try:
            ...
            if queued_jobs:
                ...
                for job in queued_jobs:
                    job_id = job["id"]
                    logger.info(
                        f"🎬 Processing job {job_id}: {job['url']} [{job['in_ts']}s - {job['out_ts']}s]"
                    )
                    ...
                    if mark_job_as_working(job_id):
                        ...
                        logger.error(
                            f"❌ Failed to mark job {job_id} as working, skipping..."
                        )
