...
class AsyncVideoProcessor:
        ...
        async with self.semaphore:
            ...
            try:
                ...
                trimmed_path = await self._trim_video_async(
                    video_path, start_time, end_time, job_id,
                ...
                self.logger.info(f"Completed async processing for job {job_id} in {processing_time:.2f}s")
                return result
                ...
                raise VideoProcessingError(f"Processing failed: {str(e)}", job_id=job_id) from e
    ...
    async def process_batch_async(
        ...
        for i in range(0, len(jobs), self.batch_size):
            ...
            self.logger.info(f"Processing batch {i//self.batch_size + 1} with {len(batch)} jobs")
    ...
    async def _download_video_async(
        ...
        return await loop.run_in_executor(
            ...
            url, job_id, format_id, progress_callback
        )
    ...
    async def _trim_video_async(
        ...
        return await loop.run_in_executor(
            ...
            video_path, start_time, end_time, job_id, progress_callback
        )
    ...
    async def _upload_video_async(
        ...
        return await loop.run_in_executor(
            None,
            self.storage_manager.store_video,
...
async def process_video_with_timeout(
    ...
    try:
        return await asyncio.wait_for(
            processor.process_video_async(**job_data),
            timeout=timeout
        )
    except asyncio.TimeoutError:
...
async def create_async_processor_pool(
    pool_size: int = 3,
    max_concurrent_per_processor: int = 5
