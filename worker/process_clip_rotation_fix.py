        ...
        for stream in metadata.get("streams", []):
            if stream.get("codec_type") == "video":
                ...
                if has_tilt_indicators:
                    logger.info(
                        f"Detected video with potential tilt issues: {width}x{height} from {handler}"
                    )

                    # Apply a slight stabilization filter that can correct minor tilts
                    # The deshake filter can correct small rotation variations
                    logger.info(
                        "Applying video stabilization to correct potential tilt"
                    )
                    return "deshake=rx=16:ry=16"

                # If no issues detected, no correction needed
                logger.info(
                    f"No rotation or tilt correction needed for {width}x{height} video"
                )
                return None
        ...
        for stream in metadata.get("streams", []):
            if stream.get("codec_type") == "video":
                ...
                if "google" in handler:
                    logger.info(
                        "Detected Google/YouTube video - applying tilt correction"
                    )
