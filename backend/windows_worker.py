...
logger.info(
    f"✅ Added paths: backend={backend_path}, project_root={project_root}, worker={worker_path}"
)
...
def create_windows_worker():
    ...
    try:
        # Create Redis connection with same settings as backend for compatibility
        redis_conn = redis_module.Redis.from_url(
            settings.redis_url,
            decode_responses=True,  # This fixes the decode() attribute error
            socket_connect_timeout=10,  # Match backend settings
            socket_timeout=10,  # Match backend settings
            retry_on_timeout=True,
            health_check_interval=30,  # Match backend settings
        )
        ...
        for attempt in range(3):
            try:
                ...
                logger.info(
                    f"✅ Redis ping successful on attempt {attempt + 1}: {result}"
                )
                break
        ...
        try:
            ...
            logger.info(
                "⚠️ Using FakeRedis for local development - jobs will work but won't persist between restarts"
            )
            ...
            logger.error(
                "❌ Both Redis and FakeRedis unavailable - fakeredis not installed"
            )
            raise Exception(
                "Redis connection failed and FakeRedis fallback unavailable"
            )
