...
class RateLimiter:
    ...
    async def check_rate_limit(self, request: Request) -> Tuple[bool, Optional[Dict[str, any]]]:
        """
        ...
        ip_bucket = self._get_or_create_bucket(self.ip_buckets, client_ip, global_config)
        ...
        if endpoint in self.rate_limits:
            ...
            if not endpoint_bucket.consume():
                return False, {
                    ...
                    'reset_in': self._calculate_reset_time(endpoint_bucket, endpoint_config),
                    'client_ip': client_ip
                }
    ...
    def _calculate_reset_time(self, bucket: TokenBucket, config: Dict[str, float]) -> int:
    ...
    async def _cleanup_old_buckets(self):
        ...
        if old_ip_keys or any(old_endpoint_keys for old_endpoint_keys in 
                             [buckets for buckets in self.endpoint_buckets.values()]):
            logger.debug(f"Cleaned up {len(old_ip_keys)} IP buckets and endpoint buckets")
    ...
    def get_rate_limit_info(self, request: Request) -> Dict[str, any]:
        ...
        endpoint_key = f"{client_ip}:{endpoint}"
        if endpoint in self.endpoint_buckets and endpoint_key in self.endpoint_buckets[endpoint]:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not is_allowed:
            ...
            headers = {
                ...
                "X-RateLimit-Reset": str(int(time.time()) + rate_limit_info['reset_in']),
        ...
        if client_ip in self.rate_limiter.ip_buckets:
            ...
            response.headers["X-RateLimit-Reset"] = str(
                int(time.time()) + self.rate_limiter._calculate_reset_time(bucket, config)
            )
