...
def init_redis():
    ...
    if redis is not None and async_redis_pool is not None:
        print(f"🔄 Redis already initialized: sync={type(redis)}, async_pool={type(async_redis_pool)}")
        return redis
    ...
    try:
        ...
        for attempt in range(3):
            try:
                result = redis.ping()
                print(f"✅ Sync Redis ping successful on attempt {attempt + 1}: {result}")
                break
        ...
        try:
            ...
            print(f"⚠️  Using FakeRedis for local development - caching will work but won't persist")
        except ImportError:
    ...
    print(f"🎯 Redis initialization complete: sync={type(redis)}, async_pool={type(async_redis_pool)}")
    return redis
