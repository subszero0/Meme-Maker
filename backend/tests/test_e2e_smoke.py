...
@pytest.mark.smoke
@pytest.mark.skip(
    reason="E2E test requires live backend server - run manually with backend running"
)
class TestE2EUserFlow:
    ...
    def test_complete_user_flow(
        self, e2e_config: dict, session: requests.Session, skip_e2e
    ):
        """
    ...
    def test_job_creation_validation(self, e2e_config: dict, session: requests.Session):
        ...
        ), f"Valid job should return 202, got {response.status_code}"
    ...
    def _fetch_metadata(
        ...
        response = session.post(f"{base_url}/api/v1/metadata", json={"url": video_url})
    ...
    def _create_job(
        ...
        job_data = {"url": video_url, "start": "00:00:00", "end": "00:00:10"}
    ...
    def _poll_until_complete(
        self, session: requests.Session, base_url: str, job_id: str, config: dict
    ) -> dict:
    ...
    def _download_clip(
        self, session: requests.Session, download_url: str, min_size: int
    ) -> int:
...
class TestAPIAvailability:
    ...
    def test_api_endpoints_reachable(self, e2e_config: dict, session: requests.Session):
        ...
        assert (
            response.status_code == 200
        ), f"Health endpoint not reachable: {response.status_code}"
