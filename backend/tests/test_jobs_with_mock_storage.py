...
class TestJobFlowWithMockStorage:
    ...
    def test_job_creation_with_mock_storage(
        ...
        with patch("app.api.jobs.q.enqueue") as mock_enqueue:
            ...
            assert (
                len(job_id) == 32
            ), f"Expected 32-char UUID hex, got {len(job_id)} chars"
            assert (
                data["status"] == "queued"
            ), f"Expected status 'queued', got '{data['status']}'"
            ...
            assert (
                args[0] == "clip_processor.process_clip"
            ), "Wrong function name enqueued"
            assert args[1] == job_id, "Wrong job_id passed to worker"
            assert (
                args[2] == "https://www.youtube.com/watch?v=BaW_jenozKc"
            ), "Wrong URL passed"
    ...
    def test_worker_upload_simulation(self, client_with_mock_storage, mock_storage):
        ...
        assert (
            object_key in mock_storage._store
        ), f"File {object_key} not found in mock storage"
        stored_content = mock_storage.get_file_content(object_key)
        assert (
            stored_content == dummy_video_data
        ), "Stored content doesn't match dummy data"
        assert len(stored_content) == len(dummy_video_data), "Content length mismatch"

    def test_job_retrieval_with_presigned_url(
        ...
        assert (
            data["job_id"] == job_id
        ), f"Job ID mismatch: expected {job_id}, got {data['job_id']}"
        assert (
            data["status"] == "done"
        ), f"Expected status 'done', got '{data['status']}'"
        assert "link" in data, "Response missing download link"

        expected_url = f"memory://{object_key}"
        assert (
            data["link"] == expected_url
        ), f"Expected URL '{expected_url}', got '{data['link']}'"
    ...
    def test_download_via_mock_url(self, client_with_mock_storage, mock_storage):
        ...
        ), f"Expected URL '{expected_url}', got '{presigned_url}'"
        ...
        assert (
            downloaded_content == dummy_video_data
        ...
        ), "Downloaded content should be bytes"
    ...
    def test_post_download_cleanup(self, client_with_mock_storage, mock_storage):
        ...
        assert (
            object_key in mock_storage._store
        ), f"File {object_key} should exist before cleanup"
        stored_keys_before = mock_storage.list_keys()
        assert (
            object_key in stored_keys_before
        ), "File should be in keys list before cleanup"
        ...
        assert (
            object_key not in mock_storage._store
        ), f"File {object_key} should be deleted after cleanup"
        stored_keys_after = mock_storage.list_keys()
        assert (
            object_key not in stored_keys_after
        ), "File should not be in keys list after cleanup"

        # Verify attempting to access deleted file raises error
        with pytest.raises(FileNotFoundError) as exc_info:
            mock_storage.get_file_content(object_key)
        assert object_key in str(
            exc_info.value
    ...
    def test_complete_job_flow_integration(
        ...
        with patch("app.api.jobs.q.enqueue") as mock_enqueue:
            ...
            ), "Job should start in queued status"
    ...
    def test_mock_storage_isolation_between_tests(self, mock_storage):
        ...
        assert (
            len(initial_keys) == 0
        ), f"Mock storage should be empty at test start, found: {initial_keys}"
