...
class TestCompleteUserJourney:
    ...
    def test_complete_user_journey(self, client):
        ...
        assert metadata_response.status_code == 200, f"Metadata fetch failed: {metadata_response.text}"
        ...
        assert job_response.status_code == 202, f"Job creation failed: {job_response.text}"
        ...
        assert "link" in final_status or "url" in final_status, "No download URL in completed job"
        ...
        assert "s3" in download_url.lower() or "amazonaws" in download_url.lower() or "localhost" in download_url.lower(), \
...
@pytest.mark.e2e
@pytest.mark.skip(
    reason="E2E test requires live backend server - run manually"
)
