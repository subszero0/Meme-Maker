...
class TestJobFlowMockStorageSimplified:
    ...
    def test_job_data_simulation_with_mock_storage(self):
        ...
        assert (
            object_key in storage.list_keys()
        ), f"Object {object_key} should be in keys list"

        # Test content retrieval
        retrieved_content = storage.get_file_content(object_key)
        assert (
            retrieved_content == dummy_video_data
        ), "Retrieved content should match original"
        assert len(retrieved_content) == len(
            dummy_video_data
    ...
    def test_presigned_url_generation_for_job(self):
        ...
        ), f"Expected {expected_url}, got {presigned_url}"
    ...
    def test_job_cleanup_simulation(self):
        ...
        assert (
            object_keys[0] not in storage.list_keys()
        ), "First file should not be in keys"
    ...
    def test_concurrent_job_simulation(self):
        ...
        for job_id, (object_key, expected_content) in job_data.items():
            ...
            ), f"Content mismatch for job {job_id}"
        ...
        for job_id, (object_key, expected_content) in job_data.items():
            if job_id not in completed_jobs:
                ...
                assert (
                    actual_content == expected_content
                ), f"Remaining job {job_id} should still work"
