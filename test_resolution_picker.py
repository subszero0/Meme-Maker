#!/usr/bin/env python3
"""
Test script to verify resolution picker functionality.

This script:
1. Tests different format_id downloads for the same video
2. Compares file properties (resolution, bitrate, file size)
3. Validates that different format_ids produce different results
4. Identifies where the resolution selection might be failing
"""
...
class VideoProperties:
    ...
    def _extract_properties(self) -> Dict:
        ...
        try:
            ...
            print(f"❌ Error extracting properties from {self.file_path}: {e}")
            return {}
    ...
    def __str__(self) -> str:
        return (f"Resolution: {self.resolution}, "
                ...
                f"FPS: {self._properties.get('fps', 0):.2f}, "
                f"Codec: {self._properties.get('codec', 'unknown')}")

def get_available_formats(url: str) -> List[Dict]:
    """Get available formats for a video URL"""
    try:
        response = requests.post(
            f"{API_BASE_URL}/metadata/extract",
            json={"url": url},
            timeout=30
        )
...
def create_clip_job(url: str, start: float, end: float, format_id: Optional[str] = None) -> Optional[str]:
    """Create a clipping job and return job ID"""
    try:
        payload = {
            "url": url,
            "in_ts": start,
...
def wait_for_job(job_id: str, timeout: int = 120) -> Optional[str]:
    """Wait for job completion and return download URL"""
...
def test_resolution_picker():
    ...
    if len(test_formats) < 2:
        print("❌ Need at least 2 different resolutions to test - not enough formats available")
        return False
