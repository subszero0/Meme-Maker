...
describe('URLInputPanel', () => {
  ...
  it('shows error when input is empty and form is submitted', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('url-error')).toHaveTextContent('Please enter a video URL');
    });
  ...
  it('shows error for invalid URLs after debounce', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('url-error')).toHaveTextContent('Please enter a valid video URL');
    }, { timeout: 500 });
  });
