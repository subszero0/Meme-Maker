...
export default function useJobPoller(
  ...
  const pollJob = useCallback(async () => {
    ...
    try {
      console.log(`📊 JobPoller: Making request to ${BASE_URL}/api/v1/jobs/${jobId}`);
      ...
      console.log(`📊 JobPoller: Job ${jobId} - Status: ${newResult.status}, Progress: ${newResult.progress}%, Stage: ${newResult.stage}`);

      setResult(newResult);

      if (jobData.status === 'done' || jobData.status === 'error') {
        console.log(`📊 JobPoller: Job ${jobId} completed with status: ${jobData.status}`);
        if (jobData.status === 'done') {
          console.log(`📊 JobPoller: Job ${jobId} SUCCESS - Download URL: ${jobData.download_url}`);
        } else {
          console.log(`📊 JobPoller: Job ${jobId} ERROR - Code: ${jobData.error_code}, Message: ${jobData.error_message}`);
        }
        cleanup();
      ...
      if (axiosError.response) {
        ...
        errorMessage = 'Backend service unavailable. Please ensure Docker services are running.';
        
        // Show help message after multiple connection failures
        if (consecutiveErrorsRef.current >= 3) {
          pushToast({
            type: 'error',
            message: 'Cannot connect to backend. Make sure Docker Desktop is running and run: docker-compose up -d'
          });
