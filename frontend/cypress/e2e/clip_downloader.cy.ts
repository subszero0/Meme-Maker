describe("Clip Downloader E2E Flow", () => {
  beforeEach(() => {
    ...
    cy.intercept("GET", "/api/v1/jobs/test123", (req) => {
      ...
      if (pollCount === 1) {
        ...
        req.reply({
          statusCode: 200,
          body: { status: "working", progress: 42 },
        });
      } else {
        req.reply({
          statusCode: 200,
          body: { status: "done", download_url: "/fixtures/final.mp4" },
        });
      }
    }).as("poll");
  ...
  it("should complete the full clip downloader flow", () => {
    ...
    cy.get('[data-testid="start-time"]').clear().realType("00:01.000");

    cy.get('[data-testid="end-time"]').clear().realType("00:05.000");

    // 7. Tick the rights checkbox
    cy.get('[data-testid="rights-checkbox"]').should("be.visible").check();
    ...
    cy.contains("File will self-destruct after this download.").should(
      "be.visible",
    );
  });
  ...
  it("should validate clip duration limits", () => {
    ...
    cy.get('[data-testid="url-input"]').realType(
    ...
    cy.get('[data-testid="start-time"]').clear().realType("00:00.000");
