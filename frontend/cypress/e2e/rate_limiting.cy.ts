describe("Rate Limiting", () => {
  ...
  it("should show rate limit notification when 429 error occurs", () => {
    ...
    cy.intercept("POST", "/api/v1/metadata", {
      ...
      body: {
        detail:
          "Rate limit exceeded. You can make 10 requests per minute. Please try again in 60 seconds.",
        retry_after: 60,
        limit_type: "global",
      },
    ...
    cy.get('[data-testid="rate-limit-notification"]').should(
      "contain",
      "Rate Limit Exceeded",
    );
    cy.get('[data-testid="rate-limit-notification"]').should(
  ...
  it("should show job creation rate limit notification", () => {
    ...
    cy.intercept("POST", "/api/v1/jobs", {
      ...
      body: {
        detail:
          "Job creation limit exceeded. You can create 3 jobs per hour. Please try again in 120 seconds.",
        retry_after: 120,
  ...
  it("should enable retry after countdown expires", () => {
    ...
    cy.intercept("POST", "/api/v1/metadata", {
      ...
      body: {
        detail:
          "Rate limit exceeded. You can make 10 requests per minute. Please try again in 2 seconds.",
        retry_after: 2,
        limit_type: "global",
      },
    }).as("rateLimitedMetadata");
  ...
  it("should allow dismissing the rate limit notification", () => {
    ...
    cy.get('[data-testid="rate-limit-notification"]')
      .contains("Dismiss")
      .click();
