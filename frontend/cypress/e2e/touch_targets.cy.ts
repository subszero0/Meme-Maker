...
describe("Mobile Touch Targets", () => {
  ...
  const verifyTouchTarget = (selector: string, elementName: string) => {
    cy.get(selector)
      ...
      .then(($el) => {
        ...
        expect(
          rect.width,
          `${elementName} width should be at least ${MINIMUM_TOUCH_SIZE}px`,
        ).to.be.at.least(MINIMUM_TOUCH_SIZE);
        expect(
          rect.height,
          `${elementName} height should be at least ${MINIMUM_TOUCH_SIZE}px`,
        ).to.be.at.least(MINIMUM_TOUCH_SIZE);
      });
  };
  ...
  describe("DownloadModal Touch Targets", () => {
    beforeEach(() => {
      ...
      cy.get('[data-cy="download-button"]', { timeout: 10000 }).should(
        "be.visible",
      );
    });
  ...
  describe("Error & Notification Touch Targets", () => {
    it("should have adequate touch targets for rate limit notification dismiss button", () => {
      ...
      cy.get('[data-cy="rate-limit-notification-dismiss"]').then(($el) => {
        if ($el.length > 0) {
          verifyTouchTarget(
            '[data-cy="rate-limit-notification-dismiss"]',
            "Rate limit dismiss button",
          );
        }
      });
    });
    ...
    it("should have adequate touch targets for queue full error dismiss button", () => {
      ...
      cy.get('[data-cy="queue-full-banner-dismiss"]').then(($el) => {
        if ($el.length > 0) {
          verifyTouchTarget(
            '[data-cy="queue-full-banner-dismiss"]',
            "Queue full dismiss button",
          );
        }
      });
    });
  ...
  describe("Accessibility & Focus Management", () => {
    it("should maintain proper focus indicators for all touch targets", () => {
      ...
      cy.get('[data-cy="url-input"]').focus().should("be.focused");
