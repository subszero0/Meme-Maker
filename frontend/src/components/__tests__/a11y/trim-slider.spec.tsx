...
describe.skip("TrimPanel Accessibility", () => {
  ...
  describe("ARIA Compliance", () => {
    it("should have no accessibility violations", async () => {
      const { container } = render(<TrimPanel {...defaultProps} />);
    ...
    it("should have correct ARIA attributes on slider handles", () => {
      ...
      expect(startHandle).toHaveAttribute(
        "aria-labelledby",
        "start-time-label",
      );
      expect(startHandle).toHaveAttribute(
      ...
      expect(endHandle).toHaveAttribute(
        "aria-describedby",
        "slider-instructions",
      );
    ...
    it('should have proper error announcements with role="alert"', async () => {
      ...
      await waitFor(() => {
        const errorMessage = screen.getByText(
          "Trim to one minute or less to proceed.",
        );
  ...
  describe("Keyboard Navigation", () => {
    ...
    it("should prevent handles from crossing over", () => {
      ...
      for (let i = 0; i < 1200; i++) {
        // Try to move to 120 seconds
        fireEvent.keyDown(startHandle, { key: "ArrowRight" });
      }

      // Start handle should stop before end handle (119.9s)
      expect(
        parseFloat(startHandle.getAttribute("aria-valuenow")!),
      ).toBeLessThan(120);

      // Move end handle near start handle
      endHandle.focus();
      for (let i = 0; i < 1200; i++) {
        // Try to move to 0 seconds
  ...
  describe("Focus Management and Visual Styling", () => {
    ...
    it("should have minimum 44x44px touch target area", () => {
      ...
      const startTouchTarget = startHandle.querySelector(
        '[aria-hidden="true"]',
      );
  ...
  describe("Screen Reader Announcements", () => {
    ...
    it("should provide helpful descriptions for all interactive elements", () => {
      ...
      expect(sliderInstructions).toHaveTextContent(
        /Dual handle slider for selecting video clip start and end times/,
      );
    ...
    it("should announce proper button state descriptions", async () => {
      ...
      expect(buttonHelp).toHaveTextContent(
  ...
  describe("Custom Step Size Support", () => {
    ...
    it("should display correct step size in instructions", () => {
      ...
      expect(
        screen.getByText(/Use arrow keys to adjust by 0.2s/),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Error State Accessibility", () => {
    ...
    it("should provide descriptive labels for duration display", () => {
      ...
      const startTimeSpan = screen.getByLabelText(
        /Current start time: 00:00.000/,
      );
