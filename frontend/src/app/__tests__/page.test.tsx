...
// Get the mocked hook
const mockUseJobPoller = useJobPoller as jest.MockedFunction<
  typeof useJobPoller
>;

const mockFetchVideoMetadata = api.fetchVideoMetadata as jest.MockedFunction<
  typeof api.fetchVideoMetadata
>;
const mockCreateJob = api.createJob as jest.MockedFunction<
  typeof api.createJob
>;

function renderWithToast(component: React.ReactElement) {
  return render(<MockToastProvider>{component}</MockToastProvider>);
}
...
describe("Home Page Flow", () => {
  ...
  it("transitions from idle to loading-metadata to trim state", async () => {
    ...
    expect(mockFetchVideoMetadata).toHaveBeenCalledWith(
  ...
  it("shows progress bar during processing", async () => {
    ...
    const progressContainer = screen
      .getByText("Processing your clip...")
      .closest("div");
  ...
  it("opens download modal when job completes", async () => {
    ...
    mockUseJobPoller.mockReturnValueOnce({ status: "queued" }).mockReturnValue({
      status: "done",
      url: "https://s3.aws.com/download-url",
    });
  ...
  it("shows queue full error when job fails with QUEUE_FULL", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Busy right now. Try again in a minute."),
      ).toBeInTheDocument();
    });
  });
  ...
  it("allows canceling from processing state", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("url-input")).toBeInTheDocument();
      expect(
        screen.queryByText("Processing your clip..."),
