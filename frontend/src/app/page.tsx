  ...
  if (
    ...
    if (pollerResult.errorCode === "QUEUE_FULL") {
      ...
      setState({
        phase: "service-error",
        error:
          "Backend services are not running. Please start Docker services.",
      });
    }
  }
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes("CORS")) {
          errorMessage =
            "CORS error: Backend not allowing frontend requests. Check server configuration.";
        } else if (
          ...
          setState({
            phase: "service-error",
            error:
              "Cannot connect to backend. Please ensure Docker Desktop is running and start services.",
          });
          return;
    ...
    try {
      ...
      if (
        ...
        setState({
          phase: "service-error",
          error:
            "Cannot connect to backend. Please ensure Docker services are running.",
        });
      } else {
        pushToast({
          type: "error",
  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto px-4 py-16">
        ...
        {/* Trim Phase */}
        {state.phase === "trim" && (
          <TrimPanel jobMeta={state.metadata} onSubmit={handleTrimSubmit} />
        )}
