...
describe('Web Share Integration Tests', () => {
  ...
  describe('NativeShareButton Component', () => {
    it('should render with file sharing capabilities detected', () => {
      ...
      expect(screen.getByText('🎉 Best experience: Native file sharing available')).toBeInTheDocument();
    });
    ...
    it('should trigger share functionality when clicked', async () => {
      ...
      const shareButton = screen.getByRole('button', { name: /share video file/i });
    ...
    it('should handle different platform capabilities gracefully', async () => {
      ...
      expect(screen.getByText('🔗 Fallback: Will open platform-specific share')).toBeInTheDocument();
    });
    ...
    it('should handle different button sizes', () => {
      ...
      let shareButton = screen.getByRole('button', { name: /share video file/i });
    ...
    it('should disable button while sharing is in progress', async () => {
      ...
      const webShareModule = await import('../lib/webShareService');
      vi.mocked(webShareModule.WebShareService.shareVideoFile).mockImplementation(
    ...
    it('should correctly adapt UI based on platform capabilities', async () => {
      const mockCapabilities = {
        ...
        recommendedApproach: 'web-share' as const,
             ...
             const webShareModule = await import('../lib/webShareService');
       vi.mocked(webShareModule.detectPlatformCapabilities).mockReturnValue(mockCapabilities);
...
describe('Web Share Service Functionality', () => {
  it('should export all required functions and classes', async () => {
    ...
    expect(typeof webShareModule.WebShareService.canShareFiles).toBe('function');
    expect(typeof webShareModule.WebShareService.shareVideoFile).toBe('function');
    expect(typeof webShareModule.WebShareService.getErrorMessage).toBe('function');
  });
