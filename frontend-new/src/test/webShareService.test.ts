...
describe('WebShareService', () => {
  ...
  describe('getErrorMessage', () => {
    it('should return appropriate error messages for different error codes', () => {
      ...
      expect(WebShareService.getErrorMessage(notSupportedError)).toContain('browser doesn\'t support');
      expect(WebShareService.getErrorMessage(userCancelledError)).toContain('cancelled');
      expect(WebShareService.getErrorMessage(downloadFailedError)).toContain('Failed to prepare');
      expect(WebShareService.getErrorMessage(shareFailedError)).toContain('Failed to open');
    });
  });
});
