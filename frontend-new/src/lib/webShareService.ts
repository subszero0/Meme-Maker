...
export class WebShareError extends Error {
  constructor(
    message: string,
    public code: 'NOT_SUPPORTED' | 'USER_CANCELLED' | 'DOWNLOAD_FAILED' | 'SHARE_FAILED',
...
export class WebShareService {
  ...
  static canShareFiles(files?: File[]): boolean {
    ...
    try {
      return navigator.canShare && files ? navigator.canShare({ files }) : false;
    } catch {
      return false;
    }
  }
  ...
  static async shareVideoFile(
    ...
    try {
      ...
      const file = await this.downloadAsFile(downloadUrl, videoTitle, onProgress);
      ...
      const webShareError = error instanceof WebShareError 
        ? error 
        : new WebShareError(
   ...
   */
  private static async shareAsLink(downloadUrl: string, videoTitle: string): Promise<void> {
...
export const detectPlatformCapabilities = () => {
  ...
  return {
    ...
    recommendedApproach: isMobile && WebShareService.isSupported() ? 'web-share' : 'download-then-share'
  };
}; 